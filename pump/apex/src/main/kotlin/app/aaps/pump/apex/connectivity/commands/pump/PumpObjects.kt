...
import app.aaps.pump.apex.connectivity.commands.device.BaseValueCommand
...
enum class PumpObject(
    ...
    companion object {
        fun findObject(commandId: CommandId, objectData: ByteArray, aapsLogger: AAPSLogger? = null): PumpObject? {
            ...
            aapsLogger?.warn(LTag.PUMPBTCOMM, "Object [0x${commandId.name}:0x${valueId.toString(16)}] not found")
            return null
        }

        fun fromDeviceCommand(command: DeviceCommand): PumpObject? {
            if (command is BaseValueCommand && command.isWrite) return CommandResponse
            if (command is GetValue) return when (command.value) {
                GetValue.Value.LatestBoluses, GetValue.Value.BolusHistory -> BolusEntry
                GetValue.Value.BasalProfiles -> BasalProfile
                GetValue.Value.StatusV1 -> StatusV1
                GetValue.Value.StatusV2 -> StatusV2
                GetValue.Value.TDDs -> TDDEntry
                GetValue.Value.WizardStatus -> WizardStatus
                GetValue.Value.Version -> FirmwareEntry
                GetValue.Value.Alarms -> AlarmEntry
                GetValue.Value.LatestTemporaryBasals -> null // TBREntry
            }
            return null
        }
    }
