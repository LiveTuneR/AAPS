    ...
    @Test fun `AutoISF short zero temps withhold equivalent basal for one fifteen twenty nine minutes`() {
        ...
        for (iobHundredths in 100..650) {
            ...
            val result = sut.determine_basal(
                ...
                true, currentTime, false, true, "AAPS", 100, 0.5, 1.0, 100, mutableListOf(), mutableListOf()
            )
            ...
            assertThat(result.rate!!).isWithin(0.0051).of((30 - minutes) / 30.0)
            assertThat(result.rate!! * 0.5).isWithin(0.0026).of((30 - minutes) / 60.0)
            assertThat(result.units).isNotNull()
            if (minutes !in found) println("TherapyReplay algorithm=AutoISF cob=20 iob=${iobHundredths / 100.0} zeroTempMinutes=$minutes oldRate=${kotlin.math.round(minutes / 30.0 * 100) / 100} newRate=${result.rate} deliveredBasal30m=${result.rate!! * 0.5} change=5082_COMPLEMENT")
            found.add(minutes)
        }
