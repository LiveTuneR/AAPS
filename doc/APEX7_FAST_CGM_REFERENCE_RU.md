...
AutoISF, графики используют Vico, а не стилизованные бары макета. Полная визуальная
приёмка на реальном устройстве остаётся открытой.

### Корректировка после просмотра пользователем

- BG dial 132 dp вместо 144; метрики минимум 42 вместо 48 dp; устройства 76
  вместо 94 dp. Активность около 36 dp вместо 68; фоновые отступы уменьшены.
  Крупным остаётся BG, без масштабирования шрифта по ширине окна.
- Собственные Android vector assets `ic_overview_pump`, `ic_overview_cannula`,
  `ic_overview_cgm` встроены в UI resources; интернет для их загрузки не нужен.
  Стандартные действия используют существующий набор Material icons.
- В Enhanced mode графики имеют правую легенду шириной 68 dp с общими границами
  для BG/IOB/COB и treatment belt. Реальные BG, IOB, basal, COB берутся из тех же
  StateFlow, что графики, без генерации значений для заполнения пропусков.
- Цель в легенде берётся из структурированного tempTargetFlow с проверкой срока.
  Зелёный диапазон low/high mark подписан отдельно: это не диапазон временной цели.
- Переключатель 3/6/12/24 часа меняет существующий Vico viewport; сохранены
  синхронизация остальных графиков и жесты. Нет изменений временных данных CGM.
- BG points уменьшены 6 -> 4 dp, непрозрачность диапазона 0.20 -> 0.07 только
  для Enhanced. Пустой treatment belt скрыт, но при событиях/режимах отображается.
- Временная цель отличается подписью/цветом; диапазон и срок окончания приходят
  из действующего MainViewModel. ISF показывает текущее значение, базовое из
  профиля и единицы, без подмены неизвестного динамического значения базовым.

Добавлены UI tests для общей правой границы легенд, выбора всех четырёх периодов
и активной временной цели со сроком. Это визуальный/host тест, не device acceptance.
